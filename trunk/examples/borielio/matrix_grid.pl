...
my @machine = qw{europa nereida beowulf orion};
...
for (@machine){
  ...
  # Condition must be more accurate
	$m->copyandmake(
	  dir        => 'matrix',
	  makeargs   => 'matrix',
	  ...
	  keepdir    => 1,
	); # unless $m->_r("matrix/$B")->result && ($m->_M("matrix/$B")->result > (-M $B));
...
for (0..$lp) {
	...
	my $m = $machine{$hn};
  my ($tc, $fc, $err);
	$pid[$_] = $m->open3($tc, $fc, $err, "./matrix $_ $np $B");
	#$pid[$_] = $m->open3($tc, $fc, $err, "./matrix $_ $np $A $B");
  $tc[$_] = $tc;

	$readset->add($fc);
	#$readset->add($err);

	my $address = refaddr($fc);
	$id{$address} = $_;
}
...
my $dim = shift @A;
my ($N, $M) = split /\s+/, $dim;
my ($chunksize, $remain) = ($N/$np, $N % $np);
my $begin = 0;
my $end = $chunksize+$remain-1; # first one makes more work
for (0..$lp) {
  my $f = $tc[$_];
  print $f $end-$begin+1," $M\n";
  print $f "@A[$begin..$end]";
  $begin = $end+1;
  $end += $chunksize;
}
