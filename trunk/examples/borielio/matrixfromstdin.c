...
int main(int argc, char *argv[])
{
	...
	if (argc != 4) {
		printf("Args: matrix <id> <N> <file B>\n");
		printf("With:\n");
		printf("\t<id> Machine number (0..N-1)\n");
		printf("\t<N> Number of machines.\n");
		/* printf("\t<file m1> Matrix A\n");
     */
		printf("\t<file m2> Matrix B\n");
    printf("\tThe first matrix will be read from STDIN\n");
		exit(2);
	}
  ...
  fflush(stdout);
	...
	m2 = matrix_read(argv[3]);
	m1 = matrix_read_stdin();
